...
--out-file ./2nd-wallet/dereg.cert
...
--certificate-file ./2nd-wallet/dereg.cert \
--certificate-script-file ./2nd-wallet/stake.script \
--mint "-$burnamount $(cat ./mint-policy/policyId).$tokenname" \
...
out1=$(expr "${utxo1[2]}" + "${utxo2[2]}" + "$deposit" - "$fee")
...
--tx-in "${utxo2[0]}#${utxo2[1]}" \
--tx-in-script-file ./2nd-wallet/payment.script \
--tx-out "$(cat ./1st-wallet/payment.addr)"+"$out1" \
--certificate-file ./2nd-wallet/dereg.cert \
--certificate-script-file ./2nd-wallet/stake.script \
--mint "-$burnamount $(cat ./mint-policy/policyId).$tokenname" \
--minting-script-file ./mint-policy/policy.script \
...
set +x
mkdir -p 06-transaction
mv tx.draft tx.raw tx.signed 06-transaction
